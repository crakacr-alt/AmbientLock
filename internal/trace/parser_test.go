...
func TestParserExtractsCapabilitiesWithoutEnvironmentValues(t *testing.T) {
	...
	if len(got.Executables) != 1 || got.Executables[0] != "/usr/bin/python3" { t.Fatalf("unexpected executables: %#v", got.Executables) }
	if len(got.Reads) != 1 || got.Reads[0] != "./config.yaml" { t.Fatalf("unexpected reads: %#v", got.Reads) }
	if len(got.Writes) != 1 || got.Writes[0] != "/var/tmp/output.txt" { t.Fatalf("unexpected writes: %#v", got.Writes) }
	if len(got.Network) != 1 || got.Network[0].Address != "203.0.113.10" || got.Network[0].Port != 443 { t.Fatalf("unexpected network: %#v", got.Network) }
	joined := strings.Join(got.EnvNames, ",")
	if joined != "PATH,API_TOKEN" && joined != "API_TOKEN,PATH" { t.Fatalf("unexpected env names: %#v", got.EnvNames) }
	if strings.Contains(joined, "super-secret") { t.Fatal("environment value leaked into observation") }
}
...
func TestParserIgnoresFailedAndKernelPseudoFileAccess(t *testing.T) {
	...
	got, err := NewParser("/work").Parse(strings.NewReader(input))
	if err != nil { t.Fatal(err) }
	if len(got.Reads) != 0 { t.Fatalf("expected no recorded reads, got %#v", got.Reads) }
}
...
func TestCreatIsWriteOnly(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if len(got.Reads) != 0 { t.Fatalf("creat must not be classified as read: %#v", got.Reads) }
	if len(got.Writes) != 1 || got.Writes[0] != "./result.txt" { t.Fatalf("unexpected writes: %#v", got.Writes) }
}
...
func TestOpenatUsesKernelResolvedPath(t *testing.T) {
	...
	got, err := NewParser("/work").Parse(strings.NewReader(input))
	if err != nil { t.Fatal(err) }
	if len(got.Reads) != 1 || got.Reads[0] != "./sub/config.yaml" { t.Fatalf("expected kernel-resolved path, got %#v", got.Reads) }
}
...
func TestOpenatDirectoryFDUsesKernelResolvedPath(t *testing.T) {
	...
	got, err := NewParser("/work").Parse(strings.NewReader(input))
	if err != nil { t.Fatal(err) }
	if len(got.Reads) != 1 || got.Reads[0] != "/opt/data/item.txt" { t.Fatalf("expected directory-fd resolved path, got %#v", got.Reads) }
}
